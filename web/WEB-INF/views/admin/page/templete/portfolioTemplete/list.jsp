	...
	<table id="dg-1" class="easyui-datagrid" title="列表" style="width: 700px; height: 300px"
		...
		<thead>
			<tr>
				...
				<th data-options="field:'portfolioType',width:80,formatter:function(value){return value.name ;}">分类</th>
			</tr>
		</thead>
	</table>
		...
		<form method="post">
			<table cellpadding="5">
	    		...
	    		<tr>
	    			<td>类型:</td>
	    			<td>
	    				<select name="portfolioType.pid">
	    					<c:forEach var="portfolioType" items="${portfolioTypeList}">
	    						<option value="${portfolioType.pid }">${portfolioType.name }</option>
							</c:forEach>
	    				</select>
	    			</td>
	    		</tr>
